...
function addMsg(user: boolean, message: string){
    ...
    if(user){
        ...
        icon.setAttribute("src", "https://cdn.jsdelivr.net/gh/beckydvn/hovor-ui/static/css/user.png");
        msgbox.appendChild(bubble);
        msgbox.appendChild(icon);
    }
    else
    {
        icon.setAttribute("src", "https://cdn.jsdelivr.net/gh/beckydvn/hovor-ui/static/css/robot.png");
        msgbox.appendChild(icon);
        msgbox.appendChild(bubble);
    }
...
function loadAllMsgs(){
    ...
    msgsJson.forEach((msg: { [x: string]: any; }) => {
        if("HOVOR" in msg){
...
async function sendUserMsg(){
    ...
    if(input.value.length > 0)
    {
        ...
        const response = await httpPost(host + "/new-message", {"user_id": getFromLocalStorage("user_id"), "msg": value})
        clearInterval(timer);
        ...
        if(response.hasOwnProperty("msg")){
            ...
            response.msg.forEach((msg: { [x: string]: any; }) => {
                addMsg(false, msg["HOVOR"]);
                msgs.push(msg);
                localStorage.setItem("msgs", JSON.stringify(msgs));
            });
